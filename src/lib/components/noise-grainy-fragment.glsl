...
uniform int highColor;
uniform int lowColor;
...
float R(int color) {
  float ret =  float((color & 0x00ff0000) >> 16);
  return ret;
}
float G(int color) {
  float ret =float((color & 0x0000ff00) >> 8);
  return ret;
}
float B(int color) {
  float ret =float((color & 0x000000ff) >> 0);
  return ret;
}
...
void main( void ) {
  ...
  float r = R(lowColor) + (R(highColor) - R(lowColor)) * total;
  float g = G(lowColor) + (G(highColor) - G(lowColor)) * total;
  float b = B(lowColor) + (B(highColor) - B(lowColor)) * total;
